...
# 4. NTP Time Server Selection (Dialog Menu)
NTP_CHOICE=$(dialog --clear \
    --backtitle "FreeBSD 15.1 Post-Installation" \
...
# 6. NTP Availability Check & Installation
NTP_CONF_FILE="/etc/ntp.conf"
if [ ! -f /usr/sbin/ntpd ] && [ ! -f /usr/local/sbin/ntpd ]; then
    log "Base NTP daemon not found. Installing ntp package via pkg..."
    pkg install -y ntp
    NTP_CONF_FILE="/usr/local/etc/ntp.conf"
    sysrc ntpd_program="/usr/local/sbin/ntpd"
else
    log "NTP daemon detected in the base system."
fi
...
# Backup original ntp.conf if not done yet
if [ -f "$NTP_CONF_FILE" ] && [ ! -f "${NTP_CONF_FILE}.bak" ]; then
    cp "$NTP_CONF_FILE" "${NTP_CONF_FILE}.bak"
fi

# Write a clean, secure, and targeted NTP configuration
cat << EOF > "$NTP_CONF_FILE"
