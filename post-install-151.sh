...
# 2. Disclaimer & Acceptance (bsddialog Menu)
bsddialog --clear \
...
# 3. Target User Selection (bsddialog Input)
...
# 4. NTP Time Server Selection (bsddialog Menu)
...
# 5. GPU Selection (bsddialog Menu)
...
# On FreeBSD 15, ntpd is in the base system. ntpdate is deprecated.
# ntpd_sync_on_start replaces the need for ntpdate at boot.
